...
export default function LoginModal({ isOpen, onClose, onLogin, onRegister }: LoginModalProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    console.log('Login submitted:', loginForm.username);
  };

  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    onRegister?.(registerForm.username, registerForm.email, registerForm.password);
    console.log('Register submitted:', registerForm.username);
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="glass-effect border-white/20 text-white max-w-md">
        <DialogHeader>
          <DialogTitle className="text-white">AnimeSite-қа қош келдіңіз</DialogTitle>
          <DialogDescription className="text-white/70">
            Аккаунтыңызға кіріңіз немесе жаңа аккаунт жасаңыз
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs defaultValue="login" className="w-full">
          ...
          <TabsContent value="login" className="space-y-4 mt-6">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-primary hover:bg-primary/90" data-testid="button-submit-login">
                Кіру
              </Button>
            </form>
          </TabsContent>
          ...
          <TabsContent value="register" className="space-y-4 mt-6">
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-primary hover:bg-primary/90" data-testid="button-submit-register">
                Тіркелу
              </Button>
            </form>
          </TabsContent>
